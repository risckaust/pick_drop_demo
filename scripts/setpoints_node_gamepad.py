...
class Controller:
    ...
    def joyCb(self, msg):
			...
			# If button RB on joystick is pressed
			if msg.buttons[5] > 0:
				self.modes.setArm()

			# If button Y on joystick is pressed
			if msg.buttons[3] > 0:
			...
			# If button LB on joystick is pressed
			if msg.buttons[4] > 0:
				self.modes.setDisarm()

    ## Update setpoint message
    def updateSp(self):
        x = self.joy_msg.axes[4]
        y = self.joy_msg.axes[3]
        z = self.joy_msg.axes[7]
